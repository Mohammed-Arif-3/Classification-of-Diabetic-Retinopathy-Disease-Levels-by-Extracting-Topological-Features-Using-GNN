...
# Define dynamic colors for background
bg_color1 = '#{0:02X}{1:02X}{2:02X}'.format(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
bg_color2 = '#{0:02X}{1:02X}{2:02X}'.format(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
main.config(bg=f"linear-gradient(to right, {bg_color1}, {bg_color2})")
...
def create_textbox(parent, height, width, x, y):
    ...
    textbox.config(bg=bg_color1, fg='white', font=('times', 12, 'bold'))
    return textbox

# Create labels
font = ('times', 16, 'bold')
title = Label(main, text='Diabetic Retinopathy Prediction', justify=LEFT)
title.config(bg='darkslategray', fg='white')  
title.config(font=font)           
title.config(height=3, width=120)       
title.pack()
